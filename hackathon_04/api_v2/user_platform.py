...
def manual_update(server_name, suff, org_id, token, kbase_list, lang_code, limit=5):
    ...
    print(kbase_list)
    kbase_id1 = kbase_list[1]
    print(kbase_id1)

    #------QA LOAD CODE----------
    q_fname = "questions_history.txt"
    a_fname = "answers_history.txt"
    batch_size = 25
...
def process_word_count(filename_in, filename_rem_words, filename_out):
    ...
    if not os.path.exists(filename_out):
        with open(filename_out, 'w') as file:
            for key in word_dict.keys():
                file.write(str(key) + "," + str(word_dict[key]) + "\n")
    else:
        ...
        with open(filename_out, 'r') as file:
            ...
            for line in lines:
                ...
                #time.sleep(1)
                if line:
                    #print("Line: ", line, "Length of line: ", len(line), "equal to ' ':", (line == ' '), "equal to 'new-line':", (line == '\n'))
                    words = line.split(',')
                    key,count = words[0],int(words[1])
                    if key in word_dict:
                        word_dict[key] += count
                    else:
                        word_dict[key] = count
...
def compute_word_frequencies():
    ...
    '''
    FOR NOW: All input text files are hard-coded!
    '''

    q_file = "questions_geography.txt"
    rem_file = "stopwords.txt"
    a_file = "freq_words_geography.txt"
    process_word_count(q_file,rem_file,a_file)

    q_file = "answers_geography.txt"
    rem_file = "stopwords.txt"
    a_file = "freq_words_geography.txt"
    process_word_count(q_file,rem_file,a_file)

    q_file = "questions_history.txt"
    rem_file = "stopwords.txt"
    a_file = "freq_words_history.txt"
    process_word_count(q_file,rem_file,a_file)
...
if __name__ == "__main__":
    ...
    #Compute word-frequencies for ALL Q/A Files!
