...
def search_trained_kbase(server_name, suff, org_id, token, kbase_list, lang_code, query, search_size=5):
    ...
    kbase_id1 = view_kbases(server_name, suff, org_id, token, limit=search_size)[0]

    print("---------------------SEARCH TRAINED KBASE AND GET THE TOP 5 ANSWERS-------------------")
    results_list = []
    confidence_list = []
    payload = {"query": query}
    results = search.search(server_name, suff, kbase_id1, payload, org_id, token, search_size=search_size)
...
def process_user_question(window, q_entry, num_answers_radio_var, chart_type_radio_var, data_type_radio_var,
    ...
    if q_entry.get():
        ...
        create_list_box(window, list_of_responses_per_training)
...
def create_list_box(window, responses):
    Lb = Listbox(window)
    ...
    for i in range(len(responses)):
        Lb.insert(i, "[" + str(i+1) + "] " + responses[i])
...
if __name__ == '__main__':
    ...
    # window = create_window('KNOWLEDGE BASE UI')
    # create_label(window, 'Ask your question...', 0.2, 0.1)
    # create_label(window, 'How many possible answers \n do you want to see?' , 0.2, 0.3)
    # q_entry = create_entry(window, 0.2, 0.175)
    # lim_entry = create_entry(window, 0.2, 0.4)
    # # canvas = create_canvas(window)
    # # list_box = create_list_box(window, entry)
    # create_button(window, '#fff000', '#ff0000', '#f0b40e', 10, 10, 1, "Go!", 0.2, 0.5,
    #               command=lambda: process_user_question(window, q_entry, lim_entry,
    #                                                     server_name, suff,
    #                                                     org_id, token, kbase_list,
    #                                                     lang_code))
    # window.mainloop()
